...
class Datamine:
    ...
    def summarize(self, hsize = 10, vsize = 10, cmcsize = 20, use_color = False):
        ...
        rows += [[color_count(len(self.by_color_inclusive[k]), use_color) for k in rows[0]]]
        printrows(padrows(rows))
        print('Breakdown by number of colors:')
        rows = [list(self.by_color_count.keys())]
        rows += [[color_count(len(self.by_color_count[k]), use_color) for k in rows[0]]]
        printrows(padrows(rows))
        print('--------------------')
        ...
        rows += [[color_count(len(self.by_type_inclusive[k]), use_color) for k in rows[0]]]
        printrows(padrows(rows))
        print('--------------------')
        ...
        d = sorted(self.by_supertype_inclusive,
                   ...
                   reverse=True)
        rows = [[k for k in d[:hsize]]]
        rows += [[color_count(len(self.by_supertype_inclusive[k]), use_color) for k in rows[0]]]
        printrows(padrows(rows))
        print('--------------------')
        ...
        rows += [[color_count(len(self.by_cmc[k]), use_color) for k in rows[0]]]
        printrows(padrows(rows))
