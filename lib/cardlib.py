...
ACTION_CATEGORIES = {
    'Removal': [
        r'\bdestroy\b', r'\bexile (target|each|all|any)\b', r'sacrifices? (a|target|an|two|three|four|five|six|X)\b',
        r'deals? [\d&^x]+ damage to (target|each|all|any|any target) (creature|planeswalker|permanent|target|any|player)',
        r'return (target|each|all|any) [^:]* to (its|their) owners?\'? hands?',
        r'gets? \-[\d&^x]+/\-[\d&^x]+'
    ],
    'Protection': [
...
def fields_from_json(src_json, linetrans = True):
    ...
    if 'rarity' in src_json:
        # Use lowercase for rarity lookup
