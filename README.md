...
### Advanced Filtering
Filter which cards the tool processes using search patterns, set codes, rarities, or even decklist files. These flags work across almost all toolkit scripts, including `encode.py`, `decode.py`, `sortcards.py`, `summarize.py`, `mtg_validate.py`, `mtg_search.py`, and `splitcards.py`.
...
```
*   **Note:** Use `ANY`, `ALL`, or `*` as the set code to search for a card name across the entire database.
...
```
*   **Fields:** `name`, `cost`, `cmc`, `type`, `supertypes`, `types`, `subtypes`, `pt`, `loyalty`, `text`, `rarity`, `mechanics`, `identity`, `id_count`, `set`, `number`, `pack`, `box`, `encoded`.
