    ...
    if getattr(args, 'json', False):
        ...
        if getattr(args, 'outfile', None):
            ...
            for c in cards:
                idx_prefix = ""
                if include_indices:
                    idx_val = str(getattr(c, '_index', ''))
                    if use_color:
                        idx_val = utils.colorize(idx_val, utils.Ansi.BOLD + utils.Ansi.YELLOW)
...
def _execute_oracle(cards, args, include_indices=False):
    ...
    if any(getattr(args, f, False) for f in search_formats) or getattr(args, 'outfile', None):
        ...
        return _execute_search(display_cards, args)
...
def handle_shell(args):
    ...
    def _resolve_indices_to_cards(index_str):
        if not last_results or not re.match(r'^[0-9,\-\s]+$', index_str):
            return []
        parts = [p.strip() for p in re.split(r',', index_str) if p.strip()]
        resolved_cards = []
        for p in parts:
            ...
            if range_match:
                ...
                for idx in indices:
                    if 1 <= idx <= len(last_results):
                        resolved_cards.append(last_results[idx-1])
            elif re.match(r'^\d+$', p):
                idx = int(p)
                if 1 <= idx <= len(last_results):
                    resolved_cards.append(last_results[idx-1])
    ...
    while True:
        try:
            ...
            resolved_cards = _resolve_indices_to_cards(line)
            if resolved_cards:
                for rc in resolved_cards:
                    o_args = copy.copy(args)
                    o_args.query = rc.name
            ...
            if line.startswith('/'):
                ...
                if cmd in ['/search', '/s']:
                    ...
                    last_results = handle_random(r_args, include_indices=True)
                elif cmd in ['/sets', '/st']:
                    st_args = copy.copy(args)
                    st_args.grep = _resolve_args(cmd_args)
                    st_args.sort = 'date'
                    ...
                    for name in resolved_names:
                        ...
                        rep_args.query = name
                        matched = handle_reprints(rep_args, include_indices=True)
                        if matched:
                            rep_results.extend(matched)
                    if rep_results:
                        last_results = rep_results
                    ...
                    for name in resolved_names:
                        sup_args = copy.copy(args)
                        sup_args.query = name
                        matched = handle_superior(sup_args, include_indices=True)
                        if matched:
                            sup_results.extend(matched)
                    if sup_results:
                        last_results = sup_results
                    ...
                    for name in resolved_names:
                        inf_args = copy.copy(args)
                        inf_args.query = name
                        matched = handle_inferior(inf_args, include_indices=True)
                        if matched:
                            inf_results.extend(matched)
                    if inf_results:
                        last_results = inf_results
                elif cmd in ['/substitutes', '/sub']:
                    resolved_names = _resolve_args(cmd_args)
                    if not resolved_names:
                    ...
                    for name in resolved_names:
                        sub_args = copy.copy(args)
                        sub_args.query = name
                        matched = handle_substitutes(sub_args, include_indices=True)
                        if matched:
                            sub_results.extend(matched)
                    if sub_results:
                    ...
                    for name in resolved_names:
                        cp_args = copy.copy(args)
                        cp_args.query = name
                        matched = handle_counterparts(cp_args, include_indices=True)
                        if matched:
                            cp_results.extend(matched)
                    if cp_results:
                        last_results = cp_results
                    ...
                    for name in resolved_names:
                        ...
                        tr_args.query = name
                        matched = handle_tribal(tr_args, include_indices=True)
                        if matched:
                            tr_results.extend(matched)
                    if tr_results:
                        last_results = tr_results
                elif cmd in ['/similar']:
                    resolved_names = _resolve_args(cmd_args)
                    if not resolved_names:
...
def handle_functional(args, include_indices=False):
    ...
    if getattr(args, 'json', False):
        ...
        return [g[0] for g in functional_reprints]
