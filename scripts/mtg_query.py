        ...
        try:
            ...
            if line.startswith('/'):
                ...
                if cmd in ['/search', '/s']:
                    ...
                    def fmt_cmd(cmds_str, desc):
                        if use_color:
                            colored_cmds = ", ".join([utils.colorize(c, utils.Ansi.CYAN) for c in cmds_str.split(", ")])
                            # Use utils.visible_len to adjust padding for ANSI codes
                            pad = " " * max(0, 20 - utils.visible_len(colored_cmds))
                            print(f"  {colored_cmds}{pad} - {desc}")
                        else:
                            print(f"  {cmds_str:<20} - {desc}")

                    print(f"  {'<card name>':<20} - Show official rules text for a specific card.")
                    fmt_cmd("/search, /s", "Search for cards matching query (displays a table).")
                    fmt_cmd("/compare, /c", "Compare multiple cards side-by-side.")
                    fmt_cmd("/superior, /sup", "Find cards generally better than the named card.")
                    fmt_cmd("/inferior, /inf", "Find cards generally worse than the named card.")
                    fmt_cmd("/random, /r", "Show random cards from the dataset.")
                    fmt_cmd("/clear", "Clear the terminal screen.")
