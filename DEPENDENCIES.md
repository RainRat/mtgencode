...
2. Extract the downloaded ZIP archive.
   ...
   **On Linux/macOS:**
   You can extract the file using the terminal:
   ```bash
   ...
   * Double-click `AllPrintings.json.zip` or right-click it and select **Extract All...**.
   * Move the extracted `AllPrintings.json` file into the `data` folder inside this project.

If you place the raw `AllPrintings.json` file into the `data/` folder, our scripts will automatically detect and load it. You do not need to specify the filename when running commands.
