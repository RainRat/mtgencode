...
class TestMTGSubset(unittest.TestCase):
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('sys.stdout', new_callable=io.StringIO)
    def test_dry_run_preview_mode(self, mock_stdout, mock_file, mock_open_file):
        mock_open_file.return_value = self.mock_cards
        ...
        for flag in ['--dry-run', '--preview', '-p']:
            ...
            test_args = ['mtg_subset.py', 'input.json', 'output.json', flag]
            with patch('sys.argv', test_args):
                mtg_subset.main()

            # Output file should NOT be opened for writing in dry-run mode
            mock_file.assert_not_called()

            out = mock_stdout.getvalue()
            self.assertIn("--- Dry Run / Preview Mode ---", out)
            self.assertIn("Total matching cards: 2", out)
            self.assertIn("MOM: 1 cards", out)
            self.assertIn("ELD: 1 cards", out)
            self.assertIn("Sample matched cards (up to 10): Card 1, Card 2", out)
