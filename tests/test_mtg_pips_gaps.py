...
import scripts.mtg_pips as mtg_pips
...
class TestMtgPipsGaps(unittest.TestCase):
    ...
    def run_main(self, args, stdin_isatty=False, stdout_isatty=False, exists_side_effect=None):
        with patch('sys.argv', ['mtg_pips.py'] + args):
    ...
    def test_pips_sorting_name(self):
        ...
        with patch('scripts.mtg_pips.jdecode.mtg_open_file', return_value=[card1, card2]):
    ...
    def test_pips_include_text(self):
        ...
        with patch('scripts.mtg_pips.jdecode.mtg_open_file', return_value=[card]):
            code, out, err = self.run_main(['testdata/uthros.json', '--json'])
            data = json.loads(out)
    ...
    def test_pips_no_matches(self):
        with patch('scripts.mtg_pips.jdecode.mtg_open_file', return_value=[]):
    ...
    def test_default_dataset_detection_local(self):
        ...
        with patch('scripts.mtg_pips.jdecode.mtg_open_file', return_value=[]) as mock_open_file:
    ...
    def test_pips_verbose(self):
        card = Card({"name": "Uthros", "manaCost": "{U}", "types": ["Spell"]})
        with patch('scripts.mtg_pips.jdecode.mtg_open_file', return_value=[card]):
            with patch('scripts.mtg_pips.open', mock_open(), create=True) as m:
