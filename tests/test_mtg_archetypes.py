...
from scripts.mtg_analyze import main as archetypes_main

class TestMtgArchetypes(unittest.TestCase):

    def test_archetypes_basic(self):
        with patch('sys.stdout', new=io.StringIO()) as fake_out:
            with patch('sys.argv', ['mtg_analyze.py', 'archetypes', 'testdata/tarkir.json', '--min-cards', '1', '--no-color']):
                archetypes_main()
    ...
    def test_archetypes_insufficient_data(self):
        with patch('sys.stderr', new=io.StringIO()) as fake_err:
            with patch('sys.argv', ['mtg_analyze.py', 'archetypes', 'testdata/tarkir.json', '--no-color']):
                archetypes_main()
    ...
    def test_archetypes_no_matches(self):
        with patch('sys.stderr', new=io.StringIO()) as fake_err:
            with patch('sys.argv', ['mtg_analyze.py', 'archetypes', 'testdata/tarkir.json', '--rarity', 'common', '--no-color']):
                archetypes_main()
                output = fake_err.getvalue()
    ...
    def test_archetypes_filtering(self):
        with patch('sys.stdout', new=io.StringIO()) as fake_out:
            with patch('sys.argv', ['mtg_analyze.py', 'archetypes', 'testdata/tarkir.json', '--grep', 'Invasion', '--min-cards', '1', '--no-color']):
                archetypes_main()
                self.assertIn("ARCHETYPE PROFILING", fake_out.getvalue())

        with patch('sys.stdout', new=io.StringIO()) as fake_out:
            with patch('sys.argv', ['mtg_analyze.py', 'archetypes', 'testdata/tarkir.json', '--limit', '1', '--min-cards', '1', '--no-color']):
                archetypes_main()
                self.assertIn("Total cards analyzed: 1", fake_out.getvalue())

    def test_archetypes_top_mechanics(self):
        with patch('sys.stdout', new=io.StringIO()) as fake_out:
            with patch('sys.argv', ['mtg_analyze.py', 'archetypes', 'testdata/tarkir.json', '--min-cards', '1', '--top-mechanics', '1', '--no-color']):
                archetypes_main()
                output = fake_out.getvalue()
    ...
    def test_archetypes_color_mock(self):
        ...
        with patch('sys.stdout', mock_stdout):
            with patch('sys.argv', ['mtg_analyze.py', 'archetypes', 'testdata/tarkir.json', '--min-cards', '1', '--color']):
                archetypes_main()
    ...
    def test_archetypes_verbose(self):
        with patch('sys.stderr', new=io.StringIO()) as fake_err:
            with patch('sys.argv', ['mtg_analyze.py', 'archetypes', 'testdata/tarkir.json', '--verbose', '--min-cards', '1', '--no-color']):
                archetypes_main()
                self.assertIn("looks like a json file", fake_err.getvalue())

    def test_archetypes_quiet(self):
        with patch('sys.stderr', new=io.StringIO()) as fake_err:
            with patch('sys.argv', ['mtg_analyze.py', 'archetypes', 'testdata/tarkir.json', '--rarity', 'common', '--quiet']):
                archetypes_main()
