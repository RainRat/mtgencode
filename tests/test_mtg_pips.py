...
def test_mtg_pips_basic():
    ...
    cmd = ["python3", "scripts/mtg_analyze.py", "pips", "testdata/uthros.json", "--no-color"]
...
def test_mtg_pips_json():
    ...
    cmd = ["python3", "scripts/mtg_analyze.py", "pips", "testdata/uthros.json", "--json"]
    result = subprocess.run(cmd, capture_output=True, text=True)
    assert result.returncode == 0
    data = json.loads(result.stdout)
    assert isinstance(data, list)
    assert any(item['sym'] == 'U' and item['cnt'] == 1 for item in data)

def test_mtg_pips_csv():
    """Test CSV output for pips."""
    cmd = ["python3", "scripts/mtg_analyze.py", "pips", "testdata/uthros.json", "--csv"]
    result = subprocess.run(cmd, capture_output=True, text=True)
    assert result.returncode == 0
...
def test_mtg_pips_include_text():
    ...
    cmd = ["python3", "scripts/mtg_analyze.py", "pips", "testdata/uthros.json", "--include-text", "--no-color"]
    result = subprocess.run(cmd, capture_output=True, text=True)
    assert result.returncode == 0
...
def test_mtg_pips_filtering():
    ...
    cmd = ["python3", "scripts/mtg_analyze.py", "pips", "testdata/uthros.json", "--set", "NONE", "--no-color"]
    result = subprocess.run(cmd, capture_output=True, text=True)
    assert result.returncode == 0
